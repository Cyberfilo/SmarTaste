"""BYOK Claude API key management endpoints.

Provides REST endpoints for storing, validating, deleting, and checking
the status of user-provided Anthropic API keys, plus cost estimation.
"""
...
async def store_key(
    ...
    """Store or update a user's Anthropic API key.
    ...
    logger.info("API key stored for user %s", current_user["user_id"])
...
async def key_status(
    ...
    """Check whether the authenticated user has a stored Claude API key.
...
async def validate_key(
    ...
    """Validate the stored Anthropic API key by making a minimal API call.

    Retrieves the user's decrypted key and calls the Anthropic API with
...
async def delete_key(
    ...
    """Remove the authenticated user's stored Anthropic API key.
    ...
    logger.info("API key removed for user %s", current_user["user_id"])
...
async def cost_estimate(
    ...
    """Return static pricing estimate for Claude API usage.

    Based on Claude Sonnet 4 pricing. This is informational only,
