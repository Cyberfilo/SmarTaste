"""Pydantic request/response models for BYOK Claude API key management endpoints."""
...
class StoreKeyRequest(BaseModel):
    """Request body for storing a user's Anthropic API key."""
...
class KeyStatusResponse(BaseModel):
    """Response indicating whether a Claude API key is configured."""

    configured: bool = Field(description="Whether an API key is stored for this user")
    masked_key: str | None = Field(
        default=None,
        description="Masked preview of the stored key (e.g., sk-ant-...XXXX)",
    )
    service: str = Field(
        default="anthropic",
...
class ValidateKeyResponse(BaseModel):
    """Response from validating an Anthropic API key."""
...
class CostEstimateResponse(BaseModel):
    """Approximate cost information for Claude API usage."""

    model: str = Field(
        default="claude-sonnet-4-20250514",
        description="Claude model used for chat",
    )
