...
import {
  ...
  useDeleteOpenAIKey,
  useOpenAICostEstimate,
} from "@/hooks/use-openai-key";
...
export function OpenAIKeyManager() {
  ...
  const { data: keyStatus, isLoading: statusLoading } = useOpenAIKeyStatus();
  const storeKey = useStoreOpenAIKey();
  const validateKey = useValidateOpenAIKey();
  const deleteKey = useDeleteOpenAIKey();
  const { data: costEstimate } = useOpenAICostEstimate();
  ...
  if (statusLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>AI Assistant (OpenAI)</CardTitle>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Bring your own OpenAI API key to use ChatGPT models
        </CardDescription>
      </CardHeader>
            ...
            {!isConfigured && (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">
                  You need an OpenAI API key to use GPT models. It takes 30 seconds:
                </p>
                <a
                ...
                <p className="text-[10px] text-muted-foreground/70">
                  Create a key &rarr; copy it &rarr; paste above &rarr; Save Key. You pay OpenAI directly per usage.
                </p>
              </div>
            )}
          </>
        ...
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will delete your stored OpenAI API key. GPT models
